...
if __name__ == "__main__":
    conversion_from, conversion_to = input(
        "請輸入想要的轉換進制 (例如 2 10 或 16 8): "
    ).split()
    conversion_from, conversion_to = int(conversion_from), int(conversion_to)

    if conversion_from not in {2, 4, 8, 10, 16} or conversion_to not in {
        2,
        4,
        8,
        10,
        ...
        print(
            f"[green]{conversion_from} 進制的 {number} 轉為 {conversion_to} 進制為: {result}[/green]"
        )
